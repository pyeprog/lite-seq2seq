...
class Seq2seq:
    ...
    def train(self, encode_file_path, decode_file_path):
        ...
        if not hasattr(self, 'sess'):
            ...
            encoder_input = tf.get_default_graph().get_tensor_by_name('inputs:0')
            encoder_input_seq_lengths = tf.get_default_graph().get_tensor_by_name('source_lens:0')
            decoder_target = tf.get_default_graph().get_tensor_by_name('targets:0')
            decoder_target_seq_lengths = tf.get_default_graph().get_tensor_by_name('target_lens:0')
    ...
    def predict(self, encode_str):
        ...
        encoder_input_seq_lengths = tf.get_default_graph().get_tensor_by_name('source_lens:0')
        decoder_target_seq_lengths = tf.get_default_graph().get_tensor_by_name('target_lens:0')
        prediction = tf.get_default_graph().get_tensor_by_name('optimization/predictions:0')
